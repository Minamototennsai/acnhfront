...
export default {
  setup() {
    ...
    // 组件创建时从/api/reactions接口获取初始数据
    onMounted(() => {
      fetchAllData();
    });
    
    // 从/api/reactions接口获取初始数据
    const fetchAllData = () => {
      axios.get('/api/reactions').then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
      });
    };    

    // 从/api/reactions/{id}接口获取搜索数据
    const fetchDataFromApiGetById = (query) => {
      axios.get(`/api/reactions/${query}`).then(response => {
        tableData.value = [response.data];
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/reactions/findByName接口获取搜索数据
    const fetchDataFromApiFindByName = (query) => {
      axios.get(`/api/reactions/findByName?name=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
