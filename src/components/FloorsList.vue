      ...
      <div>
        ...
        <div v-if="materialslist.length && materials.length">
          ...
          <ul class="materials-list">
            <li v-for="floors in materialslist" :key="floors.id" class="materials-item">
              <p>{{ floors.name }} : {{ floors.numberOfMaterial1 }} {{ floors.material1 }} {{ floors.numberOfMaterial2 }} {{ floors.material2 }} {{ floors.numberOfMaterial3 }} {{ floors.material3 }} {{ floors.numberOfMaterial4 }} {{ floors.material4 }} {{ floors.numberOfMaterial5 }} {{ floors.material5 }} {{ floors.numberOfMaterial6 }} {{ floors.material6 }}</p >
            </li>
          </ul>
        </div>
  ...
  setup() {
    ...
    const findMaterials = async () => {
      ...
      try {
        ...
        const response = await axios.get('/api/floors/materials', { params });
    ...
    // 组件创建时从/api/floors接口获取初始数据
    onMounted(() => {
      fetchAllData();
    });
    
    // 从/api/floors接口获取初始数据
    const fetchAllData = () => {
    ...
    // 从/api/floors/{id}接口获取搜索数据
    const fetchDataFromApiGetById = (query) => {
      axios.get(`/api/floors/${query}`).then(response => {
        tableData.value = [response.data];
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/floors/findByName接口获取搜索数据
    const fetchDataFromApiFindByName = (query) => {
      axios.get(`/api/floors/findByName?name=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/floors/searchBuy接口获取搜索数据
    const fetchDataFromApiSearchBuy = (min, max, sort) => {
      axios.get(`/api/floors/searchBuy?min=${min}&max=${max}&sort=${sort}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length;
        currentPage.value = 1; 
      });
    ...
    // 从/api/floors/searchSell接口获取搜索数据
    const fetchDataFromApiSearchSell = (min, max, sort) => {
      axios.get(`/api/floors/searchSell?min=${min}&max=${max}&sort=${sort}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length;
        currentPage.value = 1; 
      });
