      ...
      <div>
        ...
        <div v-if="materialslist.length && materials.length">
          ...
          <ul class="materials-list">
            <li v-for="headwear in materialslist" :key="headwear.id" class="materials-item">
              <p>{{ headwear.name }} : {{ headwear.numberOfMaterial1 }} {{ headwear.material1 }} {{ headwear.numberOfMaterial2 }} {{ headwear.material2 }} {{ headwear.numberOfMaterial3 }} {{ headwear.material3 }} {{ headwear.numberOfMaterial4 }} {{ headwear.material4 }} {{ headwear.numberOfMaterial5 }} {{ headwear.material5 }} {{ headwear.numberOfMaterial6 }} {{ headwear.material6 }}</p >
            </li>
          </ul>
        </div>
      ...
      <el-table :data="paginatedData" style="width: 100%">
        ...
        <el-table-column prop="id" label="Id">
          <template #header>
            <div>
              Id
              <el-input type="number" min="1" v-model="searchInputs.id" placeholder="Search in Id" @input="onIdSearchInput('id')" @keyup.enter="search" :disabled="isNameActive || isVariationActive || isDiyActive || isBuyActive || isSellActive || isColor1Active || isColor2Active || isMilesPriceActive || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isTypeActive || isVillagerEquippableActive || isCatalogActive"></el-input>
            </div>
          </template>
        ...
        <el-table-column prop="buy" label="Buy">
          <template #header>
            <div>
              Buy
              <div>
                <el-input type="number" min="0" v-model.number="searchInputs.buy_min" placeholder="Min" @input="onBuySearchInput('buy')" @keyup.enter="search" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isSellActive || isColor1Active || isColor2Active || isMilesPriceActive || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isTypeActive || isVillagerEquippableActive || isCatalogActive"></el-input>
                <el-input type="number" min="0" v-model.number="searchInputs.buy_max" placeholder="Max" @input="onBuySearchInput('buy')" @keyup.enter="search" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isSellActive || isColor1Active || isColor2Active || isMilesPriceActive || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isTypeActive || isVillagerEquippableActive || isCatalogActive"></el-input>
                <el-select v-model="searchInputs.buy_sort" placeholder="Sort" @change="() => {onBuySearchInput('buy'); search()}" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isSellActive || isColor1Active || isColor2Active || isMilesPriceActive || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isTypeActive || isVillagerEquippableActive || isCatalogActive">
        ...
        <el-table-column prop="sell" label="Sell">
          <template #header>
            <div>
              Sell
              <div>
                <el-input type="number" min="0" v-model.number="searchInputs.sell_min" placeholder="Min" @input="onSellSearchInput('sell')" @keyup.enter="search" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isBuyActive || isColor1Active || isColor2Active || isMilesPriceActive || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isTypeActive || isVillagerEquippableActive || isCatalogActive"></el-input>
                <el-input type="number" min="0" v-model.number="searchInputs.sell_max" placeholder="Max" @input="onSellSearchInput('sell')" @keyup.enter="search" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isBuyActive || isColor1Active || isColor2Active || isMilesPriceActive || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isTypeActive || isVillagerEquippableActive || isCatalogActive"></el-input>
                <el-select v-model="searchInputs.sell_sort" placeholder="Sort" @change="() => {onSellSearchInput('sell'); search()}" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isBuyActive || isColor1Active || isColor2Active || isMilesPriceActive || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isTypeActive || isVillagerEquippableActive || isCatalogActive">
                  <el-option label="不排序" value=""></el-option>
                  <el-option label="升序" value="asc"></el-option>
                  <el-option label="降序" value="desc"></el-option>
                </el-select>
        ...
        <el-table-column prop="source" label="Source">
          <template #header>
            <div>
              ...
              <el-option label="International Children's Day" value="International Children's Day"></el-option>        
              <el-option label="Labelle" value="Labelle"></el-option>        
              <el-option label="New Year's Eve" value="New Year's Eve"></el-option>        
              ...
              <el-option label="Summer Solstice Festival" value="Summer Solstice Festival"></el-option>        
        ...
        <el-table-column prop="type" label="Type">
          <template #header>
            <div>
              Type
              <el-select v-model="searchInputs.type" placeholder="Select" @change="() => {onTypeSearchInput('type'); search()}" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isBuyActive || isSellActive || isColor1Active || isColor2Active || isMilesPriceActive || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isVillagerEquippableActive || isCatalogActive">
              <el-option label="Null" value=""></el-option>
              <el-option label="HeadCap" value="HeadCap"></el-option>        
              <el-option label="HeadFullFace" value="HeadFullFace"></el-option>        
              <el-option label="Headgear_HasBang" value="Headgear_HasBang"></el-option>        
              <el-option label="Headgear_HasEar" value="Headgear_HasEar"></el-option>        
              <el-option label="Headgear_NoEar" value="Headgear_NoEar"></el-option>        
              <el-option label="Headgear_NoEarNoJaw" value="Headgear_NoEarNoJaw"></el-option>        
              <el-option label="HeadHairOrnament_Back" value="HeadHairOrnament_Back"></el-option>        
              <el-option label="HeadHairOrnament_Front" value="HeadHairOrnament_Front"></el-option>        
              <el-option label="HeadHairOrnament_Left" value="HeadHairOrnament_Left"></el-option>        
              <el-option label="HeadHairOrnament_Peak" value="HeadHairOrnament_Peak"></el-option>        
              <el-option label="HeadHairOrnament_Top" value="HeadHairOrnament_Top"></el-option>        
            </el-select>
            </div>
          </template>
          <template #default="{ row }">
        ...
        <!-- Villager Equippable列 -->
        <el-table-column prop="villagerEquippable" label="Villager Equippable">
          <template #header>
            <div>
              Villager Equippable
              <el-select v-model="searchInputs.villagerEquippable" placeholder="Select" @change="() => {onVillagerEquippableSearchInput('villagerEquippable'); search()}" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isBuyActive || isSellActive || isColor1Active || isColor2Active || isMilesPriceActive || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isTypeActive || isCatalogActive">
              <el-option label="Null" value=""></el-option>
              <el-option label="Yes" value="Yes"></el-option>        
              <el-option label="No" value="No"></el-option>        
            </el-select>
          ...
          <template #default="{ row }">
            <span>{{ row.villagerEquippable }}</span>
          </template>
        </el-table-column>
  ...
  setup() {
    ...
    const findMaterials = async () => {
      ...
      try {
        ...
        const response = await axios.get('/api/headwear/materials', { params });
    ...
    const isTypeActive = ref(false);
    const isVillagerEquippableActive = ref(false);
    ...
    // 组件创建时从/api/headwear接口获取初始数据
    onMounted(() => {
      fetchAllData();
    });
    
    // 从/api/headwear接口获取初始数据
    const fetchAllData = () => {
      axios.get('/api/headwear').then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
      });
    };    

    // 从/api/headwear/{id}接口获取搜索数据
    const fetchDataFromApiGetById = (query) => {
      axios.get(`/api/headwear/${query}`).then(response => {
        tableData.value = [response.data];
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/findByName接口获取搜索数据
    const fetchDataFromApiFindByName = (query) => {
      axios.get(`/api/headwear/findByName?name=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchVariation接口获取搜索数据
    const fetchDataFromApiSearchVariation = (query) => {
      axios.get(`/api/headwear/searchVariation?variation=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchDiy接口获取搜索数据
    const fetchDataFromApiSearchDiy = (query) => {
      axios.get(`/api/headwear/searchDiy?diy=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchBuy接口获取搜索数据
    const fetchDataFromApiSearchBuy = (min, max, sort) => {
      axios.get(`/api/headwear/searchBuy?min=${min}&max=${max}&sort=${sort}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length;
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchSell接口获取搜索数据
    const fetchDataFromApiSearchSell = (min, max, sort) => {
      axios.get(`/api/headwear/searchSell?min=${min}&max=${max}&sort=${sort}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length;
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchColor1接口获取搜索数据
    const fetchDataFromApiSearchColor1 = (query) => {
      axios.get(`/api/headwear/searchColor1?color1=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchColor2接口获取搜索数据
    const fetchDataFromApiSearchColor2 = (query) => {
      axios.get(`/api/headwear/searchColor2?color2=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchMilesPrice接口获取搜索数据
    const fetchDataFromApiSearchMilesPrice = (min, max, sort) => {
      axios.get(`/api/headwear/searchMilesPrice?min=${min}&max=${max}&sort=${sort}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length;
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchSource接口获取搜索数据
    const fetchDataFromApiSearchSource = (query) => {
      axios.get(`/api/headwear/searchSource?source=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchSeasonalAvailability接口获取搜索数据
    const fetchDataFromApiSearchSeasonalAvailability = (query) => {
      axios.get(`/api/headwear/searchSeasonalAvailability?seasonalAvailability=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchMannequinPiece接口获取搜索数据
    const fetchDataFromApiSearchMannequinPiece = (query) => {
      axios.get(`/api/headwear/searchMannequinPiece?mannequinPiece=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchStyle接口获取搜索数据
    const fetchDataFromApiSearchStyle = (query) => {
      axios.get(`/api/headwear/searchStyle?style=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchLabelThemes接口获取搜索数据
    const fetchDataFromApiSearchLabelThemes = (query) => {
      axios.get(`/api/headwear/searchLabelThemes?labelThemes=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchType接口获取搜索数据
    const fetchDataFromApiSearchType = (query) => {
      axios.get(`/api/headwear/searchType?type=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchVillagerEquippable接口获取搜索数据
    const fetchDataFromApiSearchVillagerEquippable = (query) => {
      axios.get(`/api/headwear/searchVillagerEquippable?villagerEquippable=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/headwear/searchCatalog接口获取搜索数据
    const fetchDataFromApiSearchCatalog = (query) => {
      axios.get(`/api/headwear/searchCatalog?catalog=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    const onTypeSearchInput = (column) => {
      if (column === 'type' && searchInputs.value.type) {
        isTypeActive.value = true;
      } else if (!searchInputs.value.type) {
        isTypeActive.value = false;
      }
    };

    const onVillagerEquippableSearchInput = (column) => {
      if (column === 'villagerEquippable' && searchInputs.value.villagerEquippable) {
        isVillagerEquippableActive.value = true;
      } else if (!searchInputs.value.villagerEquippable) {
        isVillagerEquippableActive.value = false;
      }
    };
    ...
    const search = () => {
      ...
      // 处理 type 列的搜索
        ...
        fetchDataFromApiSearchType(searchInputs.value.type);
      }

      // 处理 villagerEquippable 列的搜索
      else if (searchInputs.value.villagerEquippable) {
        fetchDataFromApiSearchVillagerEquippable(searchInputs.value.villagerEquippable);
      }
    ...
    return {
      ...
      isTypeActive,
      onTypeSearchInput,
      fetchDataFromApiSearchType,
      isVillagerEquippableActive,
      onVillagerEquippableSearchInput,
      fetchDataFromApiSearchVillagerEquippable,
      isCatalogActive,
