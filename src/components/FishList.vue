  ...
  <div>
    ...
    <div class="content">
      ...
      <el-table :data="paginatedData" style="width: 100%">
        ...
        <!-- Where How列 -->
        <el-table-column prop="whereHow" label="Where How">
          <template #header>
            Where How
          </template>
          <template #default="{ row }">
            <span>{{ row.whereHow }}</span>
          </template>
        </el-table-column>
        ...
        <!-- Rain Snow Catch Up列 -->
        <el-table-column prop="rainSnowCatchUp" label="Rain Snow Catch Up">
          <template #header>
            Rain Snow Catch Up
          </template>
          <template #default="{ row }">
            <span>{{ row.rainSnowCatchUp }}</span>
          </template>
        </el-table-column>
...
export default {
  setup() {
    ...
    // 从/api/fish/{id}接口获取搜索数据
    const fetchDataFromApiGetById = (query) => {
      axios.get(`/api/fish/${query}`).then(response => {
        tableData.value = [response.data];
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/fish/findByName接口获取搜索数据
    const fetchDataFromApiFindByName = (query) => {
      axios.get(`/api/fish/findByName?name=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/fish/searchSell接口获取搜索数据
    const fetchDataFromApiSearchSell = (min, max, sort) => {
      axios.get(`/api/fish/searchSell?min=${min}&max=${max}&sort=${sort}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length;
        currentPage.value = 1; 
      });
