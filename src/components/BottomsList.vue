      ...
      <div>
        ...
        <div v-if="materialslist.length && materials.length">
          ...
          <ul class="materials-list">
            <li v-for="bottoms in materialslist" :key="bottoms.id" class="materials-item">
              <p>{{ bottoms.name }} : {{ bottoms.numberOfMaterial1 }} {{ bottoms.material1 }} {{ bottoms.numberOfMaterial2 }} {{ bottoms.material2 }} {{ bottoms.numberOfMaterial3 }} {{ bottoms.material3 }} {{ bottoms.numberOfMaterial4 }} {{ bottoms.material4 }} {{ bottoms.numberOfMaterial5 }} {{ bottoms.material5 }} {{ bottoms.numberOfMaterial6 }} {{ bottoms.material6 }}</p >
            </li>
          </ul>
        </div>
        ...
        <el-table-column prop="buy" label="Buy">
          <template #header>
            <div>
              Buy
              <div>
                <el-input type="number" min="0" v-model.number="searchInputs.buy_min" placeholder="Min" @input="onBuySearchInput('buy')" @keyup.enter="search" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isSellActive || isColor1Active || isColor2Active || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isCatalogActive"></el-input>
                <el-input type="number" min="0" v-model.number="searchInputs.buy_max" placeholder="Max" @input="onBuySearchInput('buy')" @keyup.enter="search" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isSellActive || isColor1Active || isColor2Active || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isCatalogActive"></el-input>
                <el-select v-model="searchInputs.buy_sort" placeholder="Sort" @change="() => {onBuySearchInput('buy'); search()}" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isSellActive || isColor1Active || isColor2Active || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isCatalogActive">
        ...
        <el-table-column prop="sell" label="Sell">
          <template #header>
            <div>
              Sell
              <div>
                <el-input type="number" min="0" v-model.number="searchInputs.sell_min" placeholder="Min" @input="onSellSearchInput('sell')" @keyup.enter="search" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isBuyActive || isColor1Active || isColor2Active || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isCatalogActive"></el-input>
                <el-input type="number" min="0" v-model.number="searchInputs.sell_max" placeholder="Max" @input="onSellSearchInput('sell')" @keyup.enter="search" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isBuyActive || isColor1Active || isColor2Active || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isCatalogActive"></el-input>
                <el-select v-model="searchInputs.sell_sort" placeholder="Sort" @change="() => {onSellSearchInput('sell'); search()}" :disabled="isIdActive || isNameActive || isVariationActive || isDiyActive || isBuyActive || isColor1Active || isColor2Active || isSourceActive || isSeasonalAvailabilityActive || isMannequinPieceActive || isStyleActive || isLabelThemesActive || isCatalogActive">
                  <el-option label="不排序" value=""></el-option>
                  <el-option label="升序" value="asc"></el-option>
                  <el-option label="降序" value="desc"></el-option>
                </el-select>
  ...
  setup() {
    ...
    const findMaterials = async () => {
      ...
      try {
        ...
        const response = await axios.get('/api/bottoms/materials', { params });
    ...
    // 组件创建时从/api/bottoms接口获取初始数据
    onMounted(() => {
      fetchAllData();
    });
    
    // 从/api/bottoms接口获取初始数据
    const fetchAllData = () => {
      axios.get('/api/bottoms').then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
      });
    };    

    // 从/api/bottoms/{id}接口获取搜索数据
    const fetchDataFromApiGetById = (query) => {
      axios.get(`/api/bottoms/${query}`).then(response => {
        tableData.value = [response.data];
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/findByName接口获取搜索数据
    const fetchDataFromApiFindByName = (query) => {
      axios.get(`/api/bottoms/findByName?name=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchVariation接口获取搜索数据
    const fetchDataFromApiSearchVariation = (query) => {
      axios.get(`/api/bottoms/searchVariation?variation=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchDiy接口获取搜索数据
    const fetchDataFromApiSearchDiy = (query) => {
      axios.get(`/api/bottoms/searchDiy?diy=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchBuy接口获取搜索数据
    const fetchDataFromApiSearchBuy = (min, max, sort) => {
      axios.get(`/api/bottoms/searchBuy?min=${min}&max=${max}&sort=${sort}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length;
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchSell接口获取搜索数据
    const fetchDataFromApiSearchSell = (min, max, sort) => {
      axios.get(`/api/bottoms/searchSell?min=${min}&max=${max}&sort=${sort}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length;
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchColor1接口获取搜索数据
    const fetchDataFromApiSearchColor1 = (query) => {
      axios.get(`/api/bottoms/searchColor1?color1=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchColor2接口获取搜索数据
    const fetchDataFromApiSearchColor2 = (query) => {
      axios.get(`/api/bottoms/searchColor2?color2=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchSource接口获取搜索数据
    const fetchDataFromApiSearchSource = (query) => {
      axios.get(`/api/bottoms/searchSource?source=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchSeasonalAvailability接口获取搜索数据
    const fetchDataFromApiSearchSeasonalAvailability = (query) => {
      axios.get(`/api/bottoms/searchSeasonalAvailability?seasonalAvailability=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchMannequinPiece接口获取搜索数据
    const fetchDataFromApiSearchMannequinPiece = (query) => {
      axios.get(`/api/bottoms/searchMannequinPiece?mannequinPiece=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchStyle接口获取搜索数据
    const fetchDataFromApiSearchStyle = (query) => {
      axios.get(`/api/bottoms/searchStyle?style=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchLabelThemes接口获取搜索数据
    const fetchDataFromApiSearchLabelThemes = (query) => {
      axios.get(`/api/bottoms/searchLabelThemes?labelThemes=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/bottoms/searchCatalog接口获取搜索数据
    const fetchDataFromApiSearchCatalog = (query) => {
      axios.get(`/api/bottoms/searchCatalog?catalog=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
