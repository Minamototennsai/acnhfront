      ...
      <div>
        ...
        <div v-if="materialslist.length && materials.length">
          ...
          <ul class="materials-list">
            <li v-for="other in materialslist" :key="other.id" class="materials-item">
              <p>{{ other.name }} : {{ other.numberOfMaterial1 }} {{ other.material1 }} {{ other.numberOfMaterial2 }} {{ other.material2 }} {{ other.numberOfMaterial3 }} {{ other.material3 }} {{ other.numberOfMaterial4 }} {{ other.material4 }} {{ other.numberOfMaterial5 }} {{ other.material5 }} {{ other.numberOfMaterial6 }} {{ other.material6 }}</p >
            </li>
          </ul>
        </div>
  ...
  setup() {
    ...
    const findMaterials = async () => {
      ...
      try {
        ...
        const response = await axios.get('/api/other/materials', { params });
    ...
    // 从/api/other/{id}接口获取搜索数据
    const fetchDataFromApiGetById = (query) => {
      axios.get(`/api/other/${query}`).then(response => {
        tableData.value = [response.data];
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/other/findByName接口获取搜索数据
    const fetchDataFromApiFindByName = (query) => {
      axios.get(`/api/other/findByName?name=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/other/searchDiy接口获取搜索数据
    const fetchDataFromApiSearchDiy = (query) => {
      axios.get(`/api/other/searchDiy?diy=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/other/searchSell接口获取搜索数据
    const fetchDataFromApiSearchSell = (min, max, sort) => {
      axios.get(`/api/other/searchSell?min=${min}&max=${max}&sort=${sort}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length;
        currentPage.value = 1; 
      });
    ...
    // 从/api/other/searchMilesPrice接口获取搜索数据
    const fetchDataFromApiSearchMilesPrice = (min, max, sort) => {
      axios.get(`/api/other/searchMilesPrice?min=${min}&max=${max}&sort=${sort}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length;
        currentPage.value = 1; 
      });
