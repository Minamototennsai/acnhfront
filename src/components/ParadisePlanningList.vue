...
export default {
  setup() {
    ...
    // 组件创建时从/api/paradise_planning接口获取初始数据
    onMounted(() => {
      fetchAllData();
    });
    
    // 从/api/paradise_planning接口获取初始数据
    const fetchAllData = () => {
      axios.get('/api/paradise_planning').then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
      });
    };    

    // 从/api/paradise_planning/{id}接口获取搜索数据
    const fetchDataFromApiGetById = (query) => {
      axios.get(`/api/paradise_planning/${query}`).then(response => {
        tableData.value = [response.data];
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
    ...
    // 从/api/paradise_planning/findByName接口获取搜索数据
    const fetchDataFromApiFindByName = (query) => {
      axios.get(`/api/paradise_planning/findByName?name=${query}`).then(response => {
        tableData.value = response.data;
        total.value = tableData.value.length; 
        currentPage.value = 1; 
      });
